import { z } from "zod";

import type { Validation } from "@/core/validation/validation";
import type { HttpResponse } from "@/infra/http/http-response";

import type { HttpRequest } from "../../http-request";
import { UserNotExistsError } from "@/domain/application/errors/user/user-not-exists";
import { makeRegisterTagUseCase } from "./factories/make-register-tag-use-case";

export class RegisterTagController {
  constructor(
    private bodyValidation: Validation<{
      name: string;
      color: string;
      userId: string;
    }>,
  ) {}

  async handle(request: HttpRequest, reply: HttpResponse) {
    try {
      const { name, color, userId } = this.bodyValidation.parse(request.body);

      const registerTagsUseCase = makeRegisterTagUseCase();

      const result = await registerTagsUseCase.execute({
        name,
        userId,
        color,
      });

      if (result.isLeft()) {
        const errorMapping = new Map([[UserNotExistsError, 404]]);

        for (const [ErrorClass, statusCode] of errorMapping) {
          if (result.value instanceof ErrorClass) {
            return reply
              .status(statusCode)
              .json({ message: result.value.message });
          }
        }
      }

      return reply.status(201).send();
    } catch (error) {
      if (error instanceof z.ZodError) {
        const errorMessages = error.errors.map((err) => err.message);
        return reply.status(400).json({
          message: errorMessages[0],
        });
      }
    }
  }
}
